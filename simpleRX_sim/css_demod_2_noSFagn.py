...
class CssDemod():
    ...
    def css_demod(self, my_channel, queue, output, queue_time): 
        ...
        # plt.figure(1)
        # # plt.plot(np.arange(0,len(queue))/self.FS, 10*np.log10(queue**2))
        # plt.plot(np.arange(0,len(queue))/self.FS, 20*np.log10(queue))
        # plt.title("Power (abs(signal)**2)")
        # plt.xlabel("Time(s)")
        # plt.ylabel('Power (dB)')
        # plt.show()
        queue_idx = 0 
...
######################################################################################################################################
    ...
    def calc_SNR(self, signal_sig, noise_sig):
        ...
        # noise_power = sum(np.power(abs(noise_sig), 2)) / (len(noise_sig))
        
        # signal_power= sum(np.power(abs(signal_sig), 2)) / (len(signal_sig)) 
