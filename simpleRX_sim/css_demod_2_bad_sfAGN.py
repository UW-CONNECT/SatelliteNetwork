...
class CssDemod():
    ...
    def css_demod(self, my_channel, queue, output, queue_time): 
        ...
        if (len(self.noise_sig) == 0):
            ...
            self.nfl = 1.09 * np.max(np.abs(np.fft.fft(self.noise_sig[:self.WINDOW_SIZE] * self.UPCHIRP)))
            print("Noise floor for SF-agnostic detection: ", self.nfl) 
        if (self.QUEUE_CNT == 0):
    ...
    def activity_detection(self, Rx_Buffer, BW, FS): 
        ...
        # nfl = 0 # noise floor ? 
        minSF = 5
        uplink_wind = self.Active_Sess_SFAgn(Rx_Buffer, int(FS/BW), minSF,2,self.nfl)
