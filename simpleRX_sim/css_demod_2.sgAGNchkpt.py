...
class CssDemod():
    ...
    def css_demod(self, my_channel, queue, output, queue_time): 
        ...
        if (len(self.noise_sig) == 0):
            ...
            self.nfl = np.max(np.abs(np.fft.fft(self.noise_sig[:self.WINDOW_SIZE]*np.conjugate(self.UPCHIRP))))
        ...
        if (len(self.LEFTOVER) > self.WINDOW_SIZE*200):
            self.LEFTOVER = self.LEFTOVER[-self.WINDOW_SIZE:]
    ...
    def activity_detection(self, Rx_Buffer, BW, FS): 
        ...
        if (len(uplink_wind) > 0):
            uplink_values = uplink_wind[-1][3:len(uplink_wind[-1])]
            print("SFAGN thing: ", uplink_wind[-1][:])
        ...
        # print(uplink_wind[-1][0])
        # print(uplink_wind)
        
        # # # plt.figure(1)
        # # # plt.plot(Rx_Buffer) 
        # # plt.axvline(uplink_wind[0][1])
        # # # plt.show()
        # print(f'\nFound {len(uplink_wind)} active Periods')
        # print("SFAgn shape: ", win.shape)
...
# ####################################################################################################################################
            ...
            # lst_ind = ind
        # # print("big_ind_arr ", big_ind_arr)
        # # print("lst_ind ", lst_ind*win_jump)
        # if len(uplink_wind) > 0:
            # for i in range(0, len(uplink_wind)):
                # uplink_wind[i][0:2] = np.multiply(uplink_wind[i][0:2], [win_jump, win_jump])
            # temp = []
        ...
        # # print(f"Maximum of current peak gain is = {max(peak_gain)}")
        # # print("big_ind_arr ", big_ind_arr)
        # # uplink_wind[-1][0] = uplink_wind[-1][0] - front_buf
        # return uplink_wind
